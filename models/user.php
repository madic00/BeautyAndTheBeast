<?php 
    ...
    class User extends DbObject{
        ...
        protected static $tableFields = array("firstName", "lastName", "email", "pass", "idRole", "city", "isVerified", "activationCode", "createdAt");
        protected static $idColumnName = "id";

        public static function verify_user($email, $pass) {
            global $db;

            // $username = $db->escape_string($username);
            // $pass = $db->escape_string($pass);

            $sql = "SELECT * FROM " . self::$dbTable . " WHERE email = '{$email}' AND pass = '{$pass}' LIMIT 1";
